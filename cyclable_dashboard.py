...
Lancer : streamlit run cyclable_dashboard.py
"""
...
st.markdown("""
<style>
  [data-testid="stMetric"] { background:#fff; border-radius:12px; padding:16px !important;
...
def load_data():
    ...
    df = pd.DataFrame(stores_raw, columns=[
        "Magasin","Rep","Type","CA","Vélos","Objectif","Gap","Confiance","Essais","Ventes","Lat","Lng","Formation"
    ])
...
with st.sidebar:
    ...
    reps = ["Tous"] + sorted(df_all["Rep"].unique().tolist())
    sel_rep = st.selectbox("👤 Commercial", reps, index=0)

    types = ["Tous"] + sorted(df_all["Type"].unique().tolist())
    sel_type = st.selectbox("🏢 Type", types, index=0)

    search = st.text_input("🔍 Rechercher", placeholder="ex: Bordeaux, Paris…")

    st.divider()
    st.caption("Source : ONE PAGE B2C-B2B FY26\nMis à jour le 23 mars 2026")

# ── Filtrage ──────────────────────────────────────────────────────────────────
df = df_all.copy()
if sel_rep != "Tous":
    df = df[df["Rep"] == sel_rep]
if sel_type != "Tous":
    df = df[df["Type"] == sel_type]
if search:
    df = df[df["Magasin"].str.lower().str.contains(search.lower())]

is_filtered = (sel_rep != "Tous" or sel_type != "Tous" or bool(search))
if is_filtered:
...
with col_h1:
    ...
    st.caption("ONE PAGE B2C-B2B FY26 · Mis à jour le 23 mars 2026")
with col_h2:
    st.markdown("### `FY26 · Mar 2026`")
...
with tabs[0]:
    ...
    obj_tot    = df["Objectif"].dropna().sum()
    gap_tot    = velos_tot - obj_tot
    essais_tot = df["Essais"].sum()
    ventes_tot = df["Ventes"].sum()
    conv_tot   = round(ventes_tot / essais_tot * 100) if essais_tot > 0 else 0
    ...
    k6.metric("Nb Magasins",       f"{len(df)}",
              f"{len(df[df.Type=='Filiale'])} filiales · {len(df[df.Type=='Franchisé'])} franchisés")
    ...
    def vs_block(sub_df, label, color):
        ca   = sub_df["CA"].sum()
        vel  = sub_df["Vélos"].sum()
        ob   = sub_df["Objectif"].dropna().sum()
        gp   = vel - ob
        ...
        n3.metric("Gap",           f"−{int(abs(gp))}")

    with col_f:
        with st.container(border=True):
            vs_block(df[df["Type"]=="Filiale"],   "🔵 Filiales",   "#3b82f6")
    with col_fr:
        with st.container(border=True):
            vs_block(df[df["Type"]=="Franchisé"], "🟡 Franchisés", "#f59e0b")
    ...
    with ch2:
        ...
        fig_vel.add_trace(go.Bar(
            name="Objectif",
            x=rep_data["Rep"], y=rep_data["Objectif"],
            marker_color=[REP_COLORS.get(r, "#94a3b8") for r in rep_data["Rep"]],
            opacity=0.25,
        ))
        fig_vel.add_trace(go.Bar(
            name="Vendus",
            x=rep_data["Rep"], y=rep_data["Vélos"],
            marker_color=[REP_COLORS.get(r, "#94a3b8") for r in rep_data["Rep"]],
            text=rep_data["Vélos"].astype(str) + " 🚲",
    ...
    for i, (rep, grp) in enumerate(df.groupby("Rep")):
        with rep_cols[i % len(rep_cols)]:
            with st.container(border=True):
                ...
                r3, r4 = st.columns(2)
                r3.metric("Objectif", int(grp["Objectif"].dropna().sum()))
                att = round(grp["Vélos"].sum() / grp["Objectif"].dropna().sum() * 100, 1) if grp["Objectif"].dropna().sum() > 0 else 0
    ...
    display_df = df[["Magasin","Rep","Type","CA","Vélos","Objectif","Gap","Confiance","Essais","Ventes","Conversion"]].copy()
    display_df["CA"] = display_df["CA"].apply(lambda x: f"{x:,} €".replace(",", " "))
    display_df["Conversion"] = display_df["Conversion"].apply(lambda x: f"{x}%" if x > 0 else "—")
    display_df["Confiance"] = display_df["Confiance"].apply(lambda x: "⭐"*int(x) if pd.notna(x) else "—")
...
with tabs[1]:
    ...
    ke4.metric("Filiale",           f"{round(fil_v/fil_e*100,1) if fil_e else 0} %", f"{int(fil_e)}E → {int(fil_v)}V")
    ke5.metric("Franchisé",         f"{round(fra_v/fra_e*100,1) if fra_e else 0} %", f"{int(fra_e)}E → {int(fra_v)}V")
    ...
    rep_agg["label"] = rep_agg.apply(lambda r: f"{r.Rep}: {r.Essais}E → {r.Ventes}V ({r.Conv}%)", axis=1)
    ...
    fig_top_e.update_layout(height=300, plot_bgcolor="white", paper_bgcolor="white",
                            xaxis_tickangle=-30)
...
with tabs[2]:
    ...
    st.caption("Données cumulées YTD au 16/03/2026 · magasins onboardés en janvier–février")

    kf1, kf2, kf3, kf4 = st.columns(4)
    kf1.metric("Vélos Vendus",    "8 🚲", "en Février 2026")
    ...
    fev_active = df_all[
        df_all["Formation"].apply(
            lambda x: x and (x.startswith("2026-01") or x.startswith("2026-02"))
        ) | df_all["Magasin"].isin(["Cyclable Le Havre","Cyclable Royan","Cyclable Saint-Étienne","Cyclable Valence","Cyclable Mulhouse"])
    ]
    fev_active = fev_active[fev_active["CA"] > 0]
    fev_data = fev_active.apply(lambda r: {
        "Magasin": r["Magasin_court"],
        "Rep": r["Rep"],
        "Essais Fév": (MONTHLY_E.get(r["Magasin"], [0]*12) or [0]*12)[4],
        "Ventes Fév": (MONTHLY_V.get(r["Magasin"], [0]*12) or [0]*12)[4],
    }, axis=1)
    st.dataframe(pd.DataFrame(list(fev_data)), use_container_width=True, hide_index=True)
...
# ══════════════════════════════════════════════════════════════════════════════
with tabs[3]:
    st.markdown("## 📅 Mars 2026 — 15 magasins actifs (1–23 mars)")
    st.caption("11 magasins actifs depuis Fév + 4 nouveaux onboardés en Mars")

    km1, km2, km3, km4 = st.columns(4)
    km1.metric("Vélos Vendus",    "9 🚲", "en Mars (1–16)")
    ...
    mar_active = df_all[
        df_all["Formation"].apply(
            lambda x: x and (
                x.startswith("2026-01") or x.startswith("2026-02") or x.startswith("2026-03")
            )
        ) | df_all["Magasin"].isin(["Cyclable Le Havre","Cyclable Royan","Cyclable Saint-Étienne","Cyclable Valence","Cyclable Mulhouse"])
    ]
    mar_active = mar_active[mar_active["Essais"] + mar_active["Ventes"] + mar_active["CA"] > 0]
    mar_data = mar_active.apply(lambda r: {
        "Magasin": r["Magasin_court"],
        "Rep": r["Rep"],
        "Essais Mar": (MONTHLY_E.get(r["Magasin"], [0]*12) or [0]*12)[5],
        "Ventes Mar": (MONTHLY_V.get(r["Magasin"], [0]*12) or [0]*12)[5],
    }, axis=1)
    st.dataframe(pd.DataFrame(list(mar_data)), use_container_width=True, hide_index=True)
...
with tabs[4]:
    ...
    map_df["info"] = map_df.apply(
        lambda r: f"{r['Magasin']}<br>Rep: {r['Rep']}<br>CA: {r['CA']:,} €<br>Vélos: {r['Vélos']}<br>Essais: {r['Essais']}".replace(",", " "),
        axis=1
    )
    ...
    with c1:
        ...
        fig_reg = px.pie(
            names=list(region_ca.keys()),
            values=list(region_ca.values()),
            title="Répartition CA HT par Région",
            hole=0.35,
        )
...
with tabs[5]:
    ...
    for _, s in df_all.iterrows():
        ...
        mv = MONTHLY_V.get(s["Magasin"], [0]*12)
        ...
        if view == "Essais":      row["Total YTD"] = ytd_e
        elif view == "Ventes":    row["Total YTD"] = ytd_v
        else:                     row["Total YTD"] = round(ytd_v/ytd_e*100) if ytd_e else None
        rows.append(row)
    ...
    def color_cell(val):
        if val is None or val == 0: return "color: #94a3b8"
        return "font-weight: bold; color: #1a7a3c"

    st.dataframe(
        df_monthly.set_index("Magasin"),
        use_container_width=True,
        column_config={
            m: st.column_config.NumberColumn(m, format="%d") for m in active_months
        }
    )
    ...
    glob_e = [sum((MONTHLY_E.get(s["Magasin"],[0]*12) or [0]*12)[i] or 0 for _,s in df_all.iterrows()) for i in range(CUR+1)]
    glob_v = [sum((MONTHLY_V.get(s["Magasin"],[0]*12) or [0]*12)[i] or 0 for _,s in df_all.iterrows()) for i in range(CUR+1)]

    c1, c2 = st.columns(2)
    with c1:
        fig_m = go.Figure()
        fig_m.add_trace(go.Bar(x=active_months, y=glob_e, name="Essais",
                                marker_color="rgba(59,130,246,.55)", text=glob_e, textposition="outside"))
        fig_m.add_trace(go.Bar(x=active_months, y=glob_v, name="Ventes",
                                marker_color="rgba(40,168,79,.8)", text=glob_v, textposition="outside"))
    ...
    with c2:
        ...
        fig_c = go.Figure()
        fig_c.add_trace(go.Scatter(x=active_months, y=cumul_v,
        ...
        fig_c.add_hline(y=449, line_dash="dash", line_color="#e67e22",
                        annotation_text="Objectif 449", annotation_position="right")
        fig_c.update_layout(title="Progression Cumulée vs Objectif (449)",
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.divider()
st.caption("🚲 Dashboard Cyclable FY26 · Source : ONE PAGE B2C-B2B FY26 · 23 mars 2026")
